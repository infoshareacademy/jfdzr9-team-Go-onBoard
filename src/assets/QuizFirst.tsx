...
function Quiz() {
  ...
  const quiz = useFirebaseFetch<QuestionesData>("quiz");
  // console.log(quiz);

  // console.log("distructure", text, options);
  // const [questions, setQuestions] = useState<Question[]>([]);
  // const [options, setOptions] = useState<Option[]>([]);
  useEffect(() => {
    console.log("QUIZ", quiz);
    const questionss = quiz.map((question) => {
      const data = question.questiones;
      return data;
    });
    if (questionss.length > 0) {
      const { text, options } = questionss;
      // setQuestions(text[2]);
      // setOptions(options[0]);
      console.log(questionss[2][0]);
      console.log("distructure", text, options);
    }
    return;
  }, [quiz]);
